...
> **docs가 아직 템플릿({...} placeholder 상태)이면** 기획 스킬로 먼저 채운다:
> `/office-hours`(→PRD) → `/autoplan`(CEO→디자인→엔지니어링 리뷰 체인) → `/spec`(→step화, 아래 C·D를 자동 수행).
> 인덱스: `.claude/skills/README.md`의 "기획 스킬" 표.
...
# 이 블록은 하네스가 step 완료 후 직접 실행하는 **결정적 AC 게이트**다 — 반드시 실행 가능해야
# 하고, <...> placeholder가 남아 있으면 게이트가 생략된다(경고 표면화).
...
- **결정적 AC 게이트** — step이 `completed`로 보고돼도 커밋 전에 `step{N}.md`의 Acceptance Criteria bash 블록을 하네스가 **직접 실행**해 exit 0을 확인한다(`AC_TIMEOUT_SECONDS`=900s). 실패하면 completed를 error로 강등하고 바깥 재시도에 에러를 피드백한다 — Joy의 "AC exit 0"도 결국 LLM 보고이므로, 최종 진실은 이 비-LLM 게이트가 만든다. (AC 블록이 없는 docs-only step은 생략, 미작성 placeholder `<...>` 잔존 시 경고 후 생략. 결과는 `[하네스·meta] AC gate: exit N`으로 chat.md에 기록)
...
**바깥 안전망**: 내부 루프가 끝나거나 세션이 죽으면(타임아웃 `TIMEOUT_SECONDS`=3600s 포함) `execute.py`가 status를 읽어 바깥 재시도·커밋·하트비트를 처리한다 → 2층 안전망. `completed` 보고는 커밋 전에 **결정적 AC 게이트**(E 참조)로 한 번 더 재검증된다.
