...
# ---------------------------------------------------------------------------
# 결정적 AC 게이트 — completed 자기보고를 하네스가 step.md의 AC 재실행으로 검증
# ---------------------------------------------------------------------------

AC_STEP_TMPL = """# Step 0: x
...
class TestExtractAcScript:
    """step.md 텍스트에서 '## Acceptance Criteria' 섹션의 bash 블록 추출."""

    def test_extracts_script(self):
        text = AC_STEP_TMPL.format(ac="echo hi\npytest -q")
        script = ex.StepExecutor._extract_ac_script(text)
        assert "echo hi" in script and "pytest -q" in script

    def test_no_ac_section_returns_none(self):
        assert ex.StepExecutor._extract_ac_script("# Step\n\n## 작업\n\nx\n") is None

    def test_section_without_code_block_returns_none(self):
        text = "# Step\n\n## Acceptance Criteria\n\n수동으로 확인한다.\n"
        assert ex.StepExecutor._extract_ac_script(text) is None

    def test_comment_only_block_returns_none(self):
        text = AC_STEP_TMPL.format(ac="# 주석뿐\n\n# 커맨드 없음")
        assert ex.StepExecutor._extract_ac_script(text) is None

    def test_block_from_other_section_not_used(self):
        text = "# Step\n\n## 작업\n\n```bash\necho wrong\n```\n\n## Acceptance Criteria\n\n없음\n"
        assert ex.StepExecutor._extract_ac_script(text) is None


class TestAcGate:
    """_run_ac_gate — LLM 자기보고 대신 하네스가 AC exit 0을 직접 확인한다."""

    def _executor_with_step(self, tmp_path, ac=None, raw_step=None):
        inst = _make_executor(tmp_path, steps=[{"step": 0, "name": "x", "status": "pending"}])
        step_md = raw_step if raw_step is not None else AC_STEP_TMPL.format(ac=ac)
        (inst._phase_dir / "step0.md").write_text(step_md, encoding="utf-8")
        return inst

    def test_exit0_passes(self, tmp_path):
        inst = self._executor_with_step(tmp_path, ac="true")
        ok, detail = inst._run_ac_gate(0)
        assert ok is True
        assert "exit 0" in detail

    def test_nonzero_fails_with_detail(self, tmp_path):
        inst = self._executor_with_step(tmp_path, ac="echo boom >&2\nexit 3")
        ok, detail = inst._run_ac_gate(0)
        assert ok is False
        assert "exit 3" in detail and "boom" in detail

    def test_no_ac_passes_without_running(self, tmp_path, monkeypatch):
        inst = self._executor_with_step(tmp_path, raw_step="# Step 0\n\n## 작업\n\n문서만.\n")
        called = []
        monkeypatch.setattr(ex.subprocess, "run", lambda *a, **k: called.append(a))
        ok, _ = inst._run_ac_gate(0)
        assert ok is True and called == []

    def test_placeholder_skips_execution_with_warning(self, tmp_path, monkeypatch, capsys):
        """템플릿 placeholder(<build cmd>)가 남은 AC는 실행하면 리다이렉트로 오작동한다 —
        실행하지 않고 통과시키되 경고를 표면화해야 한다."""
        inst = self._executor_with_step(tmp_path, ac="<build cmd>     # 예: npm run build\n<test cmd>")
        called = []
        monkeypatch.setattr(ex.subprocess, "run", lambda *a, **k: called.append(a))
        ok, detail = inst._run_ac_gate(0)
        assert ok is True and called == []
        assert "placeholder" in (capsys.readouterr().out + detail).lower()

    def test_missing_step_file_passes(self, tmp_path):
        inst = _make_executor(tmp_path, steps=[{"step": 0, "name": "x", "status": "pending"}])
        ok, _ = inst._run_ac_gate(0)   # step0.md 자체가 없음 → 게이트 생략
        assert ok is True

    def test_timeout_fails(self, tmp_path, monkeypatch):
        inst = self._executor_with_step(tmp_path, ac="sleep 999")
    ...
    def test_gate_appends_chat_meta(self, tmp_path):
        inst = self._executor_with_step(tmp_path, ac="true")
        inst._run_ac_gate(0)
        chat = inst._chat_path().read_text(encoding="utf-8")
        assert "[하네스·meta] AC gate: exit 0" in chat


class TestAcGateDemotion:
    """completed 자기보고 + AC 게이트 실패 → 재시도로 강등, 최종 error."""

    def _wire(self, tmp_path, ac):
        inst = _make_executor(tmp_path, steps=[{"step": 0, "name": "x", "status": "pending"}])
        (inst._phase_dir / "step0.md").write_text(AC_STEP_TMPL.format(ac=ac), encoding="utf-8")
        inst._quiet = True                      # 인라인 tailer 생략
        inst._commits = []
        inst._commit_step = lambda *a, **k: inst._commits.append(k.get("success", True))
        inst._update_top_index = lambda *_: None
        inst._write_heartbeat = lambda *a, **k: None

        calls = {"n": 0}

        def fake_invoke(step, preamble):
            calls["n"] += 1
            idx = json.loads(inst._index_file.read_text(encoding="utf-8"))
            idx["steps"][0]["status"] = "completed"   # 세션은 매번 completed로 자기보고
            inst._index_file.write_text(json.dumps(idx, ensure_ascii=False))
            return {"step": 0, "name": "x", "exitCode": 0, "detail": "", "stdout": "", "stderr": ""}

        inst._invoke_claude = fake_invoke
        return inst, calls

    def test_failing_gate_demotes_to_error_after_retries(self, tmp_path):
        inst, calls = self._wire(tmp_path, ac="exit 1")
        step = json.loads(inst._index_file.read_text(encoding="utf-8"))["steps"][0]
        with pytest.raises(SystemExit):
            inst._execute_single_step(step, "guard")
        idx = json.loads(inst._index_file.read_text(encoding="utf-8"))
        assert idx["steps"][0]["status"] == "error"
        assert "AC" in idx["steps"][0]["error_message"]
        assert calls["n"] == ex.StepExecutor.OUTER_ATTEMPTS   # 강등 → 바깥 재시도 소진
        assert inst._commits == [False]                        # 최종 실패는 wip 커밋

    def test_passing_gate_completes_normally(self, tmp_path):
        inst, calls = self._wire(tmp_path, ac="true")
        step = json.loads(inst._index_file.read_text(encoding="utf-8"))["steps"][0]
        assert inst._execute_single_step(step, "guard") is True
        idx = json.loads(inst._index_file.read_text(encoding="utf-8"))
        assert idx["steps"][0]["status"] == "completed"
        assert calls["n"] == 1 and inst._commits == [True]
