...
팀 에이전트(**Max·Patrick·Joy·Esther**)의 craft 역량을 프레임워크에 **동봉**한 것이다.
...
하네스 **앞단의 기획 레이어**. [garrytan/gstack](https://github.com/garrytan/gstack)(MIT — 각 디렉토리에
`LICENSE.txt` 동봉)에서 기획 스킬만 가져와 텔레메트리·자동업데이트·`~/.gstack` 전역 상태·외부 CLI
의존을 전부 제거하고, 산출물을 이 저장소의 `docs/` 템플릿과 하네스 phase/step으로 재배선했다.
사용자 대면 질문·보고는 전부 한국어다(각 스킬 상단 안내 블록에 명시).

| 스킬 | 무엇 | 산출물 |
|---|---|---|
| `office-hours/` | 아이디어를 YC 오피스아워식으로 심문 → 설계문서 | `docs/PRD.md` (+ `docs/ADR.md` 초안) |
| `plan-ceo-review/` | CEO 전략 챌린지 — 4가지 scope 모드 | PRD의 P0/P1/P2·scope 갱신 + ADR |
| `plan-design-review/` | 디자이너 감사 — 차원별 0-10 채점 (UI scope 시) | `docs/UI_GUIDE.md` |
| `plan-eng-review/` | 아키텍처 락인 — 코드 정독 의무·테스트 플랜 | `docs/ARCHITECTURE.md` + `docs/ADR.md` |
| `autoplan/` | CEO→디자인→엔지니어링 리뷰 자동 체인 (취향 판단만 질문) | 위 리뷰들의 일괄 수행 |
| `spec/` | 5단계 정밀 스펙화 → **하네스 phase/step 파일 생성** | `phases/<task>/index.json`·`step{N}.md` |

**기획 파이프라인**: `/office-hours`(PRD) → `/autoplan`(리뷰 체인, 또는 `/plan-*-review` 개별) →
`/spec`(step화) → `python3 scripts/run.py <task>`(하네스 실행). 모든 파일 쓰기는 사용자 승인 후.
기획 스킬은 방법론 보존을 위해 본문 영어 원문을 유지한다(craft 스킬의 <500줄 원칙 대신 각색 시 줄인 분량 유지·추가 비대화 금지).
